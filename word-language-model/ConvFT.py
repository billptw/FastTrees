...
class ONLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, chunk_size, dropconnect=0.):
        ...
        self.ih = nn.Sequential(nn.Linear(input_size, 4 * hidden_size + self.n_chunk * 2, bias=True))
    ...
    def forward(self, input, hidden, transformed_input=None, 
                gates=None, cingate=None, cforgetgate=None):

        hx, cx = hidden
        
        cingate, cforgetgate = gates[:, :self.n_chunk*2].chunk(2, 1)            
        gates = gates[:,self.n_chunk*2:] + self.hh(hx)
        outgate, cell, ingate, forgetgate = gates.view(-1, self.n_chunk*4, self.chunk_size).chunk(4,1)
...
class LSTMStack(nn.Module):
    def __init__(self, layer_sizes, chunk_size, dropout=0., dropconnect=0.):
        ...
        # self.convnet = nn.ModuleList([TemporalBlock(layer_sizes[i], 
        #                                                 layer_sizes[i],
        #                                                 kernel_size=2, 
        #                                                 dilation=1, 
        #                                                 stride=1, 
        ...
        self.convnet = nn.ModuleList([TemporalBlock(int(layer_sizes[i+1]/chunk_size)*2, 
                                                    int(layer_sizes[i+1]/chunk_size)*2,
                                                    kernel_size=2, 
                                                    dilation=2, 
                                                    stride=1, 
                                                    padding=1) 
    ...
    def forward(self, input, hidden):
        ...
        for l in range(len(self.cells)):
            ...
            master = prev_layer[:,:,:self.cells[l].n_chunk*2]
            master = torch.transpose(master, 2, 1)
            master = self.convnet[l](master)
            master = torch.transpose(master, 2, 1)
            ifo = prev_layer[:,:,self.cells[l].n_chunk*2:]
            gates = torch.cat([master, ifo], 2)

            
            gates = self.cells[l].ih(gates)

            # chunk gates outside for slight speed-up
            # cingate, cforgetgate = gates[:, :, :self.cells[l].n_chunk*2].chunk(2, 2)
            ...
            for t in range(length):
                hidden, cell, d = self.cells[l](
                    ...
                    gates = gates[t]
                    # cingate = cingate[t], cforgetgate = cforgetgate[t] # chunk gates outside for slight speed-up
                )      
