...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, chunk_size, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        if rnn_type == 'ON':
            print('Model: ON-LSTM')
            from ON_LSTM import LSTMStack
        elif rnn_type == 'FT':
            print('Model: FastTrees')
            from FastTrees import LSTMStack
        elif rnn_type == 'FFT':
            print('Model: Faster-FastTrees')
            from FasterFT import LSTMStack
        elif rnn_type == 'CFT':
            print('Model: Conv-FastTrees')
            from ConvFT import LSTMStack
        elif rnn_type == 'LSTM':
            print('Model: LSTM')
            from LSTM import LSTMStack
        else:
